...
Camera::Camera(Ft245Interface& ft245)
    : m_verbosity{Verbosity::info},
      m_coolerOn{false},
      ft245_{ft245},
      m_ad{},
      m_thermistors{{fpga::thermistor_id::ambient,
      ...
      m_pid{0.0, 255.0, -42.0, -0.3,
            0.0},  // PID regulator: min, max, kp, ki, kd
      m_pidOutPercent{-1.0},
      m_imageData{},
      m_rawPixelData{} {
...
void Camera::connect() {
   int status = ft245_.Open();
   if (status != 0) {
      throw std::runtime_error("Unable to connect to camera");
   }
   if (!ft245_.WriteByte(fpga::command::reset)) {
      throw std::runtime_error("Unable to reset camera");
   }

   // Configure the AD9826
   ft245_.Write({
       fpga::command::rw_adconf, fpga::ad9826::cmd::write_config,
       0b11011000  // 4 V input, internal Vref, 1 Ch mode, CDS, 4V clamp
   });
   // Read back the result, same number of bytes. Discard with the
   // result.
   auto data{ft245_.Read(3)};
   this->setGain(35);  // default gain
}

// void Camera::disconnect() { ft245_.close(); }
...
//        gain,
//    };
//    bool ok = ft245_.write(writeBuffer, nBytes);
...
//        fpga::command::set_register, fpga::reg_addr::ccd_readout_mode,
//        fpga::ccd_readout_mode::bin1x1, fpga::command::toggle_read_ccd};
//    std::cout << "Initiating Readout\n";
//    bool ok = ft245_.write(writeBuffer, nBytesWrite);
...
//       size_t bytesRead = ft245_.read(buffer, chunk);
...
//       auto packet =
//           this->decodePacket(m_rawPixelData[3 * i], m_rawPixelData[3 * i +
//           1],
//                              m_rawPixelData[3 * i + 2]);
...
//    const unsigned char writeBuffer[nBytes] = {
//        fpga::command::set_register, fpga::reg_addr::ccd_readout_mode,
//        fpga::ccd_readout_mode::flush, fpga::command::toggle_read_ccd};
//    bool ok = ft245_.write(writeBuffer, nBytes);
// }

// fpga::DataPacket Camera::decodePacket(const unsigned char byte1,
//                                       const unsigned char byte2,
//                                       const unsigned char byte3) {
//    if (m_verbosity == Verbosity::debug) {
//       std::cout << "DEBUG: "
//                 << " Got bytes " << std::bitset<8>(byte1) << '_'
//                 << std::bitset<8>(byte2) << '_' << std::bitset<8>(byte3);
//    }
...
//    } else {
//       std::cout << "Error: "
//                 << " Got unknown topic byte: " << std::bitset<8>(byte1) <<
//                 '\n';
//    }
...
//       m_ad.gainVolt =
//           6.0 / (1.0 + 5.0 * (63 - static_cast<double>(data)) / 63.0);
//    } else if (address == fpga::ad9826::reg::offset) {
//       m_ad.offsetInteger = data;
//       m_ad.offsetMilliVolt =
//           1.17647058823529411765 *
...
//    for (int i = 0; i < 4; ++i) {
//       auto pkt = this->decodePacket(buffer[i * 3], buffer[i * 3 + 1],
//                                     buffer[i * 3 + 2]);
...
//    ft245_.writeByte(fpga::command::toggle_mcp);
...
//          if (pkt.topic == fpga::DataTopic::adconf)
//             topic = "adconf";
//          else if (pkt.topic == fpga::DataTopic::pixel)
//             topic = "pixel";
//          std::cout << "DEBUG: "
...
//           now,
//           this->m_thermistors.at(fpga::thermistor_id::ccd).getCelsius());
//       m_pidOutPercent =
//           pidOut / (m_pid.getMaximum() - m_pid.getMinimum()) * 100.0;
...
//    return ft245_.writeByte(fpga::command::open_shutter) == 1;
// }

// bool Camera::closeShutter() {
//    return ft245_.writeByte(fpga::command::close_shutter) == 1;
// }
...
//    return ft245_.write(writeBuffer, 3);
// }
...
// void Camera::setTemperature(double celsius) { m_pid.setTarget(celsius); }
